...
{
	internal class BuffServiceSocial : bGamesModService
	{
		private BuffWindowSocial _buffWindow;
		...
		public BuffServiceSocial()
		{
			this._hotbarIcon = new UIImage(bGamesMod.instance.Assets.Request<Texture2D>("Images/bGames/social", AssetRequestMode.ImmediateLoad));
			this.HotbarIcon.Tooltip = bGamesMod.HeroText("OpenBuffSocial");
		...
		private void HotbarIcon_onLeftClick(object sender, EventArgs e)
		{
			...
			if (_buffWindow == null)
			{
				if (!Main.dedServ)
				{
					_buffWindow = new BuffWindowSocial(Login.UserID);
	...
	internal class BuffWindowSocial : UIWindow
	{
		private UILabel lblPoints;
		private int puntos;
		private string userID;

		public BuffWindowSocial(string userID)
		{
			this.userID = userID;
			this.CanMove = true;
			UILabel lTitle = new UILabel(bGamesMod.HeroText("BuffsSocial"));
			...
			float yPos = Spacing;
			int[] socialBuffs = new int[] {
			BuffID.PetBunny, BuffID.BabyPenguin, BuffID.PetTurtle, BuffID.BabyEater, BuffID.BabySkeletronHead, BuffID.BabyHornet,
			BuffID.TikiSpirit, BuffID.PetLizard, BuffID.PetParrot, BuffID.BabyTruffle, BuffID.PetSapling, BuffID.Wisp,
			BuffID.BabyDinosaur, BuffID.BabySlime, BuffID.EyeballSpring, BuffID.BabySnowman, BuffID.FairyBlue, BuffID.FairyRed,
			BuffID.FairyGreen, BuffID.Pygmies, BuffID.Puppy, BuffID.BabyGrinch, BuffID.HornetMinion, BuffID.ImpMinion,
			BuffID.ZephyrFish, BuffID.SpiderMinion, BuffID.TwinEyesMinion, BuffID.PirateMinion, BuffID.MiniMinotaur, BuffID.SharknadoMinion,
			BuffID.UFOMinion, BuffID.DeadlySphere, BuffID.StardustMinion, BuffID.StardustGuardianMinion, BuffID.StardustDragonMinion,
			BuffID.PetDD2Gato, BuffID.PetDD2Ghost, BuffID.PetDD2Dragon, BuffID.BatOfLight, BuffID.VampireFrog, BuffID.BabyBird,
			BuffID.UpbeatStar, BuffID.SugarGlider, BuffID.SharkPup
		};

			foreach (int buffType in socialBuffs)
			{
				if (Main.debuff[buffType] && buffType != BuffID.Wet) continue;
