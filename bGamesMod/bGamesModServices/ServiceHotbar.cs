...
using bGamesMod.UIKit.UIComponents;
...
namespace bGamesMod.bGamesModServices
{
	internal class ServiceHotbar : UIWindow
	{
		...
		{
			bGamesMod.ServiceController.ServiceAdded += ServiceAddedOrRemoved;
			bGamesMod.ServiceController.ServiceRemoved += ServiceAddedOrRemoved;
			InitUI();
		}

		// Recalculate buttons.
		private void ServiceAddedOrRemoved(bGamesModService modifiedService)
		{
			// Clear existing icons in the Hotbar
			_iconView.RemoveAllChildren();
			// For each service, add its icon to the hotbar
			float xPos = Spacing;
			for (int i = 0; i < bGamesMod.ServiceController.Services.Count; i++)
			{
				bGamesModService service = bGamesMod.ServiceController.Services[i];
				...
				if (service.IsInHotbar/* && service.HotbarParent.buttonView != null*/)
				{
					...
					UIImage icon = bGamesMod.ServiceController.Services[i].HotbarIcon;
		...
		private void InitUI()
		{
			...
			collapseButton = new UIImage(bGamesMod.instance.Assets.Request<Texture2D>("Images/CollapseBar/CollapseButtonHorizontal", AssetRequestMode.ImmediateLoad));
			collapseButton.UpdateWhenOutOfBounds = true;
			collapseArrow = new UIImage(bGamesMod.instance.Assets.Request<Texture2D>("Images/CollapseBar/CollapseArrowHorizontal", AssetRequestMode.ImmediateLoad));
