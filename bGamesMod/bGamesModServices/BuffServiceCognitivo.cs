...
{
	internal class BuffServiceCognitivo : bGamesModService
	{
		private BuffWindowCognitivo _buffWindow;
		...
		public BuffServiceCognitivo()
		{
			this._hotbarIcon = new UIImage(bGamesMod.instance.Assets.Request<Texture2D>("Images/bGames/cognitive", AssetRequestMode.ImmediateLoad));
			this.HotbarIcon.Tooltip = bGamesMod.HeroText("OpenBuffCognitivo");
		...
		private void HotbarIcon_onLeftClick(object sender, EventArgs e)
		{
			...
			if (_buffWindow == null)
			{
				if (!Main.dedServ)
				{
					_buffWindow = new BuffWindowCognitivo(Login.UserID);
	...
	internal class BuffWindowCognitivo : UIWindow
	{
		private UILabel lblPoints;
		private int puntos;
		private string userID;

		public BuffWindowCognitivo(string userID)
		{
			this.userID = userID;
			this.CanMove = true;
			UILabel lTitle = new UILabel(bGamesMod.HeroText("BuffsCognitivo"));
			...
			float yPos = Spacing;
			int[] cognitiveBuffs = new int[] {
			BuffID.ManaRegeneration, BuffID.MagicPower, BuffID.Spelunker, BuffID.Invisibility, BuffID.Shine, BuffID.NightOwl,
			BuffID.Hunter, BuffID.Dangersense, BuffID.Clairvoyance, BuffID.DryadsWard, BuffID.MagicLantern, BuffID.Sharpened,
			BuffID.Bewitched, BuffID.SoulDrain, BuffID.ShadowOrb, BuffID.DryadsWard, BuffID.MagicLantern, BuffID.Sharpened,
			BuffID.Bewitched, BuffID.NebulaUpLife1, BuffID.NebulaUpLife2, BuffID.NebulaUpLife3, BuffID.NebulaUpMana1, BuffID.NebulaUpMana2,
			BuffID.NebulaUpMana3, BuffID.NebulaUpDmg1, BuffID.NebulaUpDmg2, BuffID.NebulaUpDmg3, BuffID.Lucky
		};

			foreach (int buffType in cognitiveBuffs)
			{
