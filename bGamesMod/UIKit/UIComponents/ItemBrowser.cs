...
namespace bGamesMod.UIKit.UIComponents
{
	internal class ItemBrowser : UIWindow
	{
		internal static string HeroText(string key, string category = "ItemBrowser") => bGamesMod.HeroText($"{category}.{key}");
		...
		public ItemBrowser()
		{
			...
			_expandTexture = bGamesMod.instance.Assets.Request<Texture2D>("Images/ExpandIcon", AssetRequestMode.ImmediateLoad);
			_collapseTexture = bGamesMod.instance.Assets.Request<Texture2D>("Images/CollapseIcon", AssetRequestMode.ImmediateLoad);
			_spacerTexture = bGamesMod.instance.Assets.Request<Texture2D>("Images/spacer", AssetRequestMode.ImmediateLoad);
			...
			_bClose = new UIImage(bGamesMod.instance.Assets.Request<Texture2D>("Images/closeButton", AssetRequestMode.ImmediateLoad));
		...
		private void PopulateSortView()
		{
			if (DefaultSorts == null)
			{
				...
				{
					new Sort( new UIImage(bGamesMod.instance.Assets.Request<Texture2D>("Images/sortItemID", AssetRequestMode.ImmediateLoad)){Tooltip = HeroText("SortName.ItemID")} , (x,y)=>x.type.CompareTo(y.type)),
					new Sort( new UIImage(bGamesMod.instance.Assets.Request<Texture2D>("Images/sortValue", AssetRequestMode.ImmediateLoad)){Tooltip = HeroText("SortName.Value")} , (x,y)=>x.value.CompareTo(y.value)),
					new Sort( new UIImage(bGamesMod.instance.Assets.Request<Texture2D>("Images/sortAZ", AssetRequestMode.ImmediateLoad)){Tooltip = HeroText("SortName.Alphabetical")} , (x,y)=>x.Name.CompareTo(y.Name)),
				};
			}
		...
		private void PopulateFilterView()
		{
			if (Filters == null)
			{
				Filters = new Filter[]
				{
					new Filter( new UIImage(bGamesMod.instance.Assets.Request<Texture2D>("Images/filterMod", AssetRequestMode.ImmediateLoad)) {Tooltip = HeroText("FilterName.ModFilter")}, x=>x.ModItem != null),
				};
			}
			//Category[] categories = Categories;
			//if (SelectedCategory != null)
		...
		public static void ParseList2()
		{
			...
			Categories = new Category[] {
				new Category("Weapons"/*, x=>x.damage>0*/) {
					SubCategories = new List<Category>() {
						...
						{
							Sorts = new Sort[] { new Sort(new UIImage(bGamesMod.instance.Assets.Request<Texture2D>("Images/sortAmmo", AssetRequestMode.ImmediateLoad)) {Tooltip = HeroText("SortName.UseAmmoType")}, (x,y)=>x.useAmmo.CompareTo(y.useAmmo)), }
						},
					...
					Sorts = new Sort[] { new Sort(new UIImage(bGamesMod.instance.Assets.Request<Texture2D>("Images/sortDamage", AssetRequestMode.ImmediateLoad)) {Tooltip = HeroText("SortName.Damage")}, (x,y)=>x.damage.CompareTo(y.damage)), }
				},
				new Category("Tools"/*,x=>x.pick>0||x.axe>0||x.hammer>0*/) {
					SubCategories = new List<Category>() {
						new Category("Pickaxes", x=>x.pick>0 && !ItemID.Sets.IsDrill[x.type] ) { Sorts = new Sort[] { new Sort(new UIImage(bGamesMod.instance.Assets.Request<Texture2D>("Images/sortPick", AssetRequestMode.ImmediateLoad)) {Tooltip = HeroText("SortName.PickPower")}, (x,y)=>x.pick.CompareTo(y.pick)), } },
						new Category("Axes", x=>x.axe>0&&!ItemID.Sets.IsChainsaw[x.type]){ Sorts = new Sort[] { new Sort(new UIImage(bGamesMod.instance.Assets.Request<Texture2D>("Images/sortAxe", AssetRequestMode.ImmediateLoad)) {Tooltip = HeroText("SortName.AxePower")}, (x,y)=>x.axe.CompareTo(y.axe)), } },
						new Category("Hammers", x=>x.hammer>0){ Sorts = new Sort[] { new Sort(new UIImage(bGamesMod.instance.Assets.Request<Texture2D>("Images/sortHammer", AssetRequestMode.ImmediateLoad)) {Tooltip = HeroText("SortName.HammerPower")}, (x,y)=>x.hammer.CompareTo(y.hammer)), } },
						new Category("Drill", x=>ItemID.Sets.IsDrill[x.type]){ Sorts = new Sort[] { new Sort(new UIImage(bGamesMod.instance.Assets.Request<Texture2D>("Images/sortPick", AssetRequestMode.ImmediateLoad)) {Tooltip = HeroText("SortName.PickPower")}, (x,y)=>x.pick.CompareTo(y.pick)), } },
						new Category("Chainsaw", x=>ItemID.Sets.IsChainsaw[x.type]){ Sorts = new Sort[] { new Sort(new UIImage(bGamesMod.instance.Assets.Request<Texture2D>("Images/sortAxe", AssetRequestMode.ImmediateLoad)) {Tooltip = HeroText("SortName.AxePower")}, (x,y)=>x.axe.CompareTo(y.axe)), } },
					},
				},
				new Category("Armor"/*,  x=>x.headSlot!=-1||x.bodySlot!=-1||x.legSlot!=-1*/) {
					...
					Sorts = new Sort[] { new Sort(new UIImage(bGamesMod.instance.Assets.Request<Texture2D>("Images/sortDefense", AssetRequestMode.ImmediateLoad)) {Tooltip = HeroText("SortName.Defense")}, (x,y)=>x.defense.CompareTo(y.defense)), }
				},
				...
				{
					Sorts = new Sort[] { new Sort(new UIImage(bGamesMod.instance.Assets.Request<Texture2D>("Images/sortAmmo", AssetRequestMode.ImmediateLoad)) {Tooltip = HeroText("SortName.AmmoType")}, (x,y)=>x.ammo.CompareTo(y.ammo)), }
				},
				...
				new Category("BossSummons", x=>ItemID.Sets.SortingPriorityBossSpawns[x.type] != -1 && !SortingPriorityBossSpawnsExclusions.Contains(x.type) || x.netID == ItemID.PirateMap || x.netID == ItemID.SnowGlobe || x.netID == ItemID.DD2ElderCrystal) { // vanilla bug.
					Sorts = new Sort[] { new Sort(new UIImage(bGamesMod.instance.Assets.Request<Texture2D>("Images/sortDamage", AssetRequestMode.ImmediateLoad)) {Tooltip = HeroText("SortName.ProgressionOrder")}, (x,y)=>ItemID.Sets.SortingPriorityBossSpawns[x.type].CompareTo(ItemID.Sets.SortingPriorityBossSpawns[y.type])), }
				},
				...
				new Category("Fishing"/*, x=> x.fishingPole > 0 || x.bait>0|| x.questItem*/){
					SubCategories = new List<Category>() {
						new Category("Poles", x=>x.fishingPole > 0) {Sorts = new Sort[] { new Sort(new UIImage(bGamesMod.instance.Assets.Request<Texture2D>("Images/sortFish", AssetRequestMode.ImmediateLoad)) {Tooltip = HeroText("SortName.PolePower")}, (x,y)=>x.fishingPole.CompareTo(y.fishingPole)), } },
						new Category("Bait", x=>x.bait>0) {Sorts = new Sort[] { new Sort(new UIImage(bGamesMod.instance.Assets.Request<Texture2D>("Images/sortBait", AssetRequestMode.ImmediateLoad)) {Tooltip = HeroText("SortName.BaitPower")}, (x,y)=>x.bait.CompareTo(y.bait)), } },
			...
			foreach (var modCallCategory in bGamesMod.instance.modCategories)
			{
	...
	public class Category
	{
		...
		{
			Name = name;
			LocalizedName = bGamesMod.HeroText($"ItemBrowser.CategoryName.{name}");
			Items = new List<Item>();
			SubCategories = new List<Category>();
			Sorts = new Sort[0];
			belongs = x => false;
		...
		{
			Name = name;
			LocalizedName = skipLocalization ? name : bGamesMod.HeroText($"ItemBrowser.CategoryName.{name}");
			Items = new List<Item>();
			SubCategories = new List<Category>();
			Sorts = new Sort[0];
			this.belongs = belongs;
